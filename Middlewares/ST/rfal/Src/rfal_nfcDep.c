...
#if( (RFAL_FEATURE_NFC_DEP_BLOCK_MAX_LEN < RFAL_FEATURE_NFC_DEP_BLOCK_MAX_LEN) )
    #error " RFAL: Invalid NFC-DEP APDU Max length. Please change RFAL_FEATURE_NFC_DEP_APDU_MAX_LEN. "
#endif   
...
#define NFCIP_MAX_RTOX_RETRYS           (uint8_t)10U     /*!< Number of retrys for RTOX    Digital 2.0 17.12.4.3    */
    ...
    if( gNfcip.rxBuf[msgIt] == (uint8_t)NFCIP_CMD_PSL_REQ )
    {
        ...
        *outBRS = gNfcip.rxBuf[msgIt++];                  /* assign output BRS value      */
        ...
        if( nfcDepDev != NULL )
        {
            ...
            nfcDepDev->info._DSI = (rfalBitRate)rfalNfcDepBRS2DSI( *outBRS );   /* DSI codes the bit rate from Initiator to Target */
...
ReturnCode rfalNfcDepInitiatorHandleActivation( rfalNfcDepAtrParam* param, rfalBitRate desiredBR, rfalNfcDepDevice* nfcDepDev )
{
    ...
    rfalGetBitRate( &nfcDepDev->info._DSI, &nfcDepDev->info.DRI );
    ...
    /*******************************************************************************/
    sendPSL = false;
    PSL_BRS = rfalNfcDepDx2BRS( nfcDepDev->info._DSI );  /* Set current bit rate divisor on both directions  */
    ...
    * */
    ...
    /* Check Baud rates                                                            */
    /*******************************************************************************/
    if( (nfcDepDev->info._DSI != desiredBR) && (desiredBR != RFAL_BR_KEEP) )    /* if desired BR is different    */
    {
    ...
    if( sendPSL )
    {
        ...
        if( nfcDepDev->info._DSI != desiredBR )
        {
            /* Check if device was in Passive NFC-A and went to higher bit rates, use NFC-F */
            if( (nfcDepDev->info._DSI == RFAL_BR_106) && (gNfcip.cfg.commMode == RFAL_NFCDEP_COMM_PASSIVE) )
            {
            ...
            nfcDepDev->info._DSI  = desiredBR;  /* DRI Bit Rate coding from Target to Initiator   */
            
            rfalSetBitRate( nfcDepDev->info._DSI, nfcDepDev->info.DRI );
        }
...
ReturnCode rfalNfcDepListenStartActivation( const rfalNfcDepTargetParam *param, const uint8_t *atrReq, uint16_t atrReqLength, rfalNfcDepListenActvParam rxParam )
{
    ...
    rfalGetBitRate( &rxParam.nfcDepDev->info._DSI, &rxParam.nfcDepDev->info.DRI );
    ...
    switch (err)
    {
        case ERR_NONE:
            ...
            if( BRS != RFAL_NFCDEP_BRS_MAINTAIN )
            {
                ...
                {
                    EXIT_ON_ERR( err, rfalSetMode( RFAL_MODE_LISTEN_ACTIVE_P2P, gNfcip.nfcDepDev->info.DRI, gNfcip.nfcDepDev->info._DSI ));
                }
                else
                {
                    EXIT_ON_ERR( err, rfalSetMode( ((RFAL_BR_106 == gNfcip.nfcDepDev->info.DRI) ? RFAL_MODE_LISTEN_NFCA : RFAL_MODE_LISTEN_NFCF), gNfcip.nfcDepDev->info.DRI, gNfcip.nfcDepDev->info._DSI ));
                }
            }
            break;
    ...
    switch( ret )
    {
        ...
        case ERR_AGAIN:       /*  PRQA S 2003 # MISRA 16.3 - Intentional fall through */
            ...
            if( (*gNfcip.PDUParam.rxLen) > 0U )    /* MISRA 21.18 */
            {
                ...
                if( (gNfcip.PDURxPos + (*gNfcip.PDUParam.rxLen)) > RFAL_FEATURE_NFC_DEP_PDU_MAX_LEN )
                {
                    return ERR_NOMEM;
                }
